    ...
    */
    ...
    for (var i = 0; i < lastId; i++) {
        if ('content' in document.createElement('template')) {
            ...
            var likesRecord = parsedPostData[i].likesRecord;
            
            console.log(likesRecord);
            console.log(likesRecord["likeRecords"][0]);
            console.log(typeof parseInt(userId));
            
            var liked = likesRecord.likeRecords.includes(parseInt(userId));
            ...
            if (liked) {
                    ...
                    likeButton.setAttribute("onclick", "dislikePost(" + parsedPostData[i].id + ")");
                } else {
                    likeButton.innerHTML = "Like";
                    likeButton.setAttribute("onclick", "likePost(" + parsedPostData[i].id + ")");
                }
            ...
            likeButton.id = 'likeButton' + parsedPostData[i].id;

            document.getElementById("post").classList.add('post' + parsedPostData[i].id);
            document.getElementById("post").setAttribute("data-postid", parsedPostData[i].id);
